...
object Anagrams {
  ...
  def combinations[A](occurrences: Occurrences): List[Occurrences] = {
  ...
  def sentenceAnagrams(s: Sentence): List[Sentence] = {
    def wordAnagrams(occurrences: Occurrences) = dictionaryByOccurrences.get(occurrences).flatten
    def find(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case _ => for (combo <- combinations(occurrences); word <- wordAnagrams(combo); subset <- find(subtract(occurrences, combo))) yield word :: subset
    }
    find(sentenceOccurrences(s))
  }
